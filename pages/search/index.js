...
Page({
  ...
  dealResult(res){
    ...
    if (res.result.data && res.result.data.length > 0) {
      ...
      that.currentPage++
      var searchResult = res.result.data
      ...
      if (searchResult.length < that.pageSize) {
        that.setData({
  ...
  searchForGoods: util.throttle(function(e){
    ...
    var searchHistory = wx.getStorageSync("searchHistory")||[]
    var index = searchHistory.indexOf(this.searchInput)
    if(index!=-1){
    ...
    searchHistory.unshift(this.searchInput)
  ...
  shortcutInput(e){
    ...
    console.log('sousuo')
    this.setData({
      searchInput:this.data.searchHistory[index]
    }) 
    this.searchInput = this.data.searchInput
    this.searchForGoods()
  },
